...
-- Toggle OpenCode in tmux pane with leader+c
vim.keymap.set('n', '<leader>c', function()
  ...
  -- Check if opencode pane exists by looking for the pane with opencode command
  ...
  if opencode_pane_id then
    -- OpenCode pane exists, just focus it
    vim.fn.system('tmux select-pane -t ' .. opencode_pane_id)
  else
    -- OpenCode pane doesn't exist, create it
...
require('lazy').setup({
  ...
  },
  {
    ...
    config = function()
      ...
      local servers = {
        ...
        ruby_lsp = {
          cmd = { 'bundle', 'exec', 'ruby-lsp' }, -- prefer project Gemfile
          filetypes = { 'ruby' },
  ...
  -- Replaced copilotchat with opencode.nvim
  {
    ...
    config = function()
      ...
      -- avoid remapping + and - to ctrl keys which may be used elsewhere
      -- increment/decrement left as default behaviour
    end,
  },
...
-- Change directory to the argument if it's a folder
